...
## 17. Сессия 2026-09-09 (P2): регрессия cd2eb4e — «не зайти в апстрим-роутер»

Симптомы: LuCI тестового роутера (192.168.3.1) доступен из LAN, а апстрим-роутер
192.168.1.1 (WAN-шлюз) — больше НЕ доступен. Это **новая** проблема, не повторение
ранее исправленных.

### 17.1 Проверка: предыдущие фиксы НЕ сломаны (cd2eb4e их не трогает)

`git show cd2eb4ed9d --stat` меняет только `mtk_offload.c` (+119/−7),
`mtk_debugfs.c` (+17/−?), `mtk_offload.h` (+10/−6). Проверено в текущем дереве:

- **Lifecycle (1ebcb8d826 / fb30bd3816)**: `nf_register_net_hook(&init_net,...)`
  вызывается ДО `mtk_ppe_debugfs_init` (`mtk_ppe_probe`, mtk_offload.c:965/972);
  `mtk_ppe_remove` снимает хук и `mtk_offload_eth = NULL` (984–985);
  `mtk_ppe_debugfs_init` использует `IS_ERR(root)` (mtk_debugfs.c:162). ✅
- **Свёртка хинта / компилятор (7eb7ab836b)**: хинт по-прежнему байтовый
  (`mtk_offload_put_hint`/`has_hint`, mtk_offload.h:329–350); sample доставляется
  через `netif_receive_skb` (mtk_eth_soc.c:958). ✅
- **RX/cb-passthrough**: без изменений относительно прошлой сессии.

`mtk_eth_soc.c` cd2eb4e не изменял вообще.

### 17.2 Корневая причина (код, `mtk_offload.c:794–933`)

Новый SDK-style хук теперь срабатывает на **каждый** IPv4 TCP/UDP пакет в
POSTROUTING, а не только на sample с хинтом. Единственный `return NF_DROP` во всём
файле (бывшая строка 933) выполнялся при **любом** успешном bind, т.е. для каждого
пакета, 5-тьюпл которого совпал со слотом `state==UNBIND`, созданным PPE.

Почему это ломает доступ именно к 192.168.1.1:

- `dst_neigh_lookup(dst, &t_other->src.u3.ip)` — это `t_other->src` = адрес удалённой
  стороны. Для доступа к самому WAN-шлюзу удалённая сторона (192.168.1.1) является
  **on-link** (ARP-сосед eth0.2) → lookup УСПЕВАЕТ (а не уходит в `sdk_neigh_fail`).
- Проходят все проверки (hash→UNBIND→tuple→neigh), запись заполняется, `state=BIND`,
  и **реальный forward-пакет получает NF_DROP**. Первый SYN/SYN-ACK/reply нового
  потока теряется → соединение к апстрим-роутеру (и любой on-link-цели) не поднимается.
- LuCI тестового роутера (192.168.3.1) — это сам роутер, forward не задействован,
  поэтому доступен (в `sdk_local`).

В старом cb-hint варианте NF_DROP получали только sample-пакеты с хинтом
(~185 за сессию, §12.19), т.е. практически только подлинные дубликаты → потерь
почти не было. В новом коде G2-ветвь (проверка хинта) удалена → дропаются ВСЕ
реальные пакеты совпавших потоков.

Косвенное подтверждение: `sdk_fwd`/`sdk_local` уже различают направление
(введено cd2eb4e), и для forward-направления в соседней сети (не on-link сервер)
neigh=miss → NF_ACCEPT, поэтому ломался именно on-link/шлюз-доступ.

### 17.3 Исправление (working tree, НЕ закоммичено)

Конец `mtk_offload_bind_hook`: bind выполняется безусловно (hash-подход не умеет
отличить sample от реального пакета), а `NF_DROP` возвращается **только** когда в
`skb->cb[44..47]` жив хинт (это и есть подлинный SDIC-дубликат — PPE уже отправил
оригинал в порт назначения):
...
```c
	...
	/* Drop only genuine SDIC samples ... */
...
Рациональность:
- `has_hint` переживает только у ~0.5% пакетов (rate-limited SDIC) → «drop при хинте»
  ≈ «drop только дубликат». Реальные пакеты (без хинта, 99%+) продолжают идти в стек.
- Если окажется, что sample = реальная копия (модель №2), потери всё равно лишь
  0.5% — как в стабильном старом варианте (185 дропов сеть не ломали).
- Полный `return NF_ACCEPT` всегда — НЕ выбран: тогда подлинные дубликаты уйдут в
  стек и на провод (двойная отправка кадров).

### 17.4 Что проверить на железе (когда тестовый роутер вернётся)

1. Подняться из LAN клиента в апстрим-роутер 192.168.1.1 (LuCI/ICMP/TCP-новые соединения).
2. `rx_reasons`: `sdk_hash_match`/`bind_hook_cnt` растут, `sdk_fwd` растёт,
   `sdk_neigh_fail` мал (только не-on-link цели).
3. Убедиться, что нет массовых потерь: торрент/iperf стабильны, отсутствуют повторы
   TCP (RETRANS) и NACK.
4. Если появятся дубликаты по проводу (или counter hint-пакетов ~0) — рассмотреть
   полный `return NF_ACCEPT` (bind без дропа) и отдельно разобрать hash vs PPE.
5. Заодно зафиксировать `sdk_not_unbind`/`sdk_hash_mismatch`: если высоки — hash не
   совпадает с HW-слотом (влияет только на offload, не на связность).

### 17.5 Изменения для коммита

```
mtk_offload.c:  comment(782–791) + конец bind_hook (NF_DROP → NF_DROP_если_хинт)
```

Собрано в текущем дереве только как правка исходника; пересборка/firmware по возврату
тестового роутера (§16.4 процедура пересборки).

## 18. Сессия 2026-09-10 (P3): коммит 259086c1f3 НЕ решил проблему — углублённый анализ

Статус на железе: с 259086c1f3 (drop только при живом хинте) доступ к апстрим-роутеру
192.168.1.1 по-прежнему не работает. Дампов счетчиков/пакетов с этого прогона НЕТ —
это понизило достоверность локализации, ниже дан перечень данных, которые нужны.

### 18.1 Главный вывод: дроп не был (единственной) причиной

| коммит | bind-частота | drop-политика | связность |
|--------|--------------|---------------|-----------|
| 7eb7ab836b | только sample с хинтом (~185/сессия) | NF_DROP при хинте | работала |
| cd2eb4ed9d | КАЖДЫЙ пакет потока | NF_DROP всем | сломалась |
| 259086c1f3 | КАЖДЫЙ пакет потока | NF_DROP при хинте | СЛОМАНА |

Различие «работает/не работает» коррелирует не с drop, а с **частотой bind**:
после 259086c1f3 дропаются по-прежнему ~0.5% (как в 7eb7ab836b), но bind по-прежнему
выполняется на КАЖДЫЙ совпавший пакет (как в cd2eb4e). Раз сеть не восстановилась —
причина не в NF_DROP, а в самом массовом bind (или в побочном эффекте хука).

### 18.2 КРИТИЧЕСКАЯ коррекция §16.3: hash НЕ симметричен

§16.3 утверждал: «hash(src,dst)==hash(dst,src), слот общий для обоих направлений».
Проверено численно (скрипт = точная формула mtk_flow_hash_v4):

```
client=192.168.3.142, server=192.168.1.1, sport=50000, dport=80
hash(orig)   = 3918
hash(reply)  = 2902        → НЕ равны
```

Причина: в формуле `hash = (ports & src)|(~ports & dst)` битовая маска ports
**направленно выбирает** между src/dst; после разворота направления меняются и
позиции портов в ports, и роли адресов — результат не коммутирует.
Следствия:
- Поток занимает ДВА разных слота: A=hash(orig-как-на-проводе), B=hash(reply-как-на-проводе).
- bind_hook связывает каждый слот своим направлением (для reply-пакета t_this=reply → слот B) —
  это согласовано и, по данным §16 (sdk_hash_match рос), формула совпадает с PPE для
  направления «как на проводе».
- Но: если PPE на самом деле использует **симметричный**/один слот на поток, то наш
  bind в слот B пишет запись, которую PPE для этого потока не смотрит, а слот A
  содержит ТОЛЬКО forward-MAC/NAT (dmac=шлюз) → reply-направление железо обрабатывает
  через слот A с MAC шлюза вместо MAC клиента → reply теряется/мисделиверится,
  соединение умирает после первых пакетов. ЭТО надо измерить на железе (тест T1).
- Доп. следствие для теста: тумблер направлений A/B виден в all_entry — по тьюплу
  записи видно, в какие слоты PPE кладёт поток.

### 18.3 Кандидаты-механизмы (что именно ломает при «bind на каждый пакет»)

| # | механизм | как проверить |
|---|----------|---------------|
| M1 | PPE использует 1 симметричный слот, а мы пишем 2 разных (A/B) — направленный мисделивери | T1 (пустой FOE + 1 поток + дамп до/после) |
| M2 | Неатомарная запись (memset→fill→state=BIND) гоняется с чтением PPE (tear → полу-BIND запись → black-hole потока) | T7 (нагрузка, запись на каждый пакет) |
| M3 | Дрейф туплов/NAT: entry->`bfib1.time_stamp` из `mtk_r32(..0x0010)` переписывается каждый пакет, но `new_sport` в REPLY-слоте при SNAT-ассигнении порта на проводе не совпадает из-за CONNTRACK | T2/T3 (сверить all_entry с tcpdump) |
| M4 | Дубликаты UNBIND-кадров ушли на провод (после 259086c1f3 sample больше НЕ дропается): HW-forward + SW-forward одного кадра | T5 (tcpdump счёт кадров SYN/данных на eth0.2) |
| M5 | Коллизия слота с уже-BIND записью другой пары протекает в `idx+1`? — в bind_hook сдвига НЕТ (а в mtk_offload_add есть ohash+1/rhash+1) | T6 (счёт sdk_not_unbind vs sdk_hash_match) |
| M6 | hash(slot) программ vs PPE для младших битов (наш `&0x7ff *2`) — при полном заполнении FOE перезапись чужих UNBIND | T1 (дамп адресов слотов) |

### 18.4 Решение-рекомендация для следующей итерации кода (после T1–T2)

- Если T1 покажет, что PPE держит поток в ДВУХ слотах (A и B) — база правильная;
  тогда причина смотреть в M2/M3/M4 (тест даст, какой).
- Если T1 покажет ОДИН слот (симметричный) — bind_hook должен писать в единственный
  слот направление «по этому пакету», а решать разворот должна HW смена MAC; тогда
  возвращаемся к однослотовой модели и чиним MAC/NAT под неё.
- Промежуточная страховка (безопасный базовый вариант): ограничить bind теми же
  пакетами, что и 7eb7ab836b (только при живом хинте) — вернуть рабочую связность,
  а SDK-механику доводить отдельно (воспроизведённый деградация-тест).

## 19. СПИСОК И МЕТОДИКА ТЕСТОВ (очередность выполнения на железе)

Стенд: тестовый роутер (весь трафик через него), клиент из LAN (WiFi 192.168.3.x,
но подойдёт и eth0.1), апстрим-роутер/сервер за WAN (eth0.2, 192.168.1.0/24).
Все прогоны — на одном boot без перезагрузки между пунктами (кроме T-control),
с сохранением дампов. Формат записи результатов — приложение.

### T0. ЭКСПОЗИЦИЯ-КОНТРОЛЬ: hook выключен (baseline)
- Вариант A (без пересборки): флаг не сделать — только вариантом B.
- Вариант B: собрать прошивку с bind_hook = `return NF_ACCEPT` в первой строке
  (или временно `nf_unregister_net_hook`), прошить.
- Проверить: доступ к 192.168.1.1, iperf3 TCP/UDP, пинг, DNS — ВСЁ должно работать
  1:1 как до всякого HWNAT. Зафиксировать rx_reasons (0x0e/0x0f всё равно растут,
  т.к. PPE шлёт sample при любом hook — это норм).
- *Выход из теста*: если и тут не работает → проблема вне HWNAT (VLAN/маршрут), стоп.
  Если работает → причина в хуке/bind, идём дальше.

### T1. СЛОТОВАЯ МОДЕЛЬ PPE (симметричный/асимметричный)
- На чистом FOE (после `echo 3 > /proc/sys/vm/drop_caches` не надо; просто после
  загрузки) стартовать ОДИН iperf3-клиент → сервер (или один curl до 192.168.1.1).
- Сразу после первых пакетов: `cat /sys/kernel/debug/mtk_ppe/all_entry | grep -i 192.168`
- Ожидания:
  - Два слота с тьюплами `клиент->сервер` и `сервер->wan_ip` (или `клиент->wan_ip`?) —
    записать их номера и сравнить с `hash(orig)`/`hash(reply)` скриптом из §18.2.
  - Один слот → симметричная модель → переход к §18.3 M1 (подобрать направление записи).
- Фиксируем: номера слотов, state до/после, тьюплы, MAC, vlan.

### T2. ОДИН ПОТОК, ИЗОЛИРОВАННО (TCP handshake)
- iperf3 из LAN на сервер за WAN, длиной 1 соединение; на роутере `tcpdump -i eth0.2 -nn port 5201` и на клиенте `tcpdump`.
- Записать: сколько SYN ушло на провод, сколько SYN-ACK пришло на клиента, когда
  появляются state=BIND в all_entry для слота A и B.
- *Диагностический критерий*: если SYN доходит, SYN-ACK теряется ВСЕГДА после появления
  BIND — это M1/M2 (железо съедает reply), если теряется и до BIND — дроп.
- Повторить для UDP (как у curl без retry — ICMP для UDP не поможет).

### T3. СВЕРКА ЗАПИСИ С КОНТАКТАМИ (правильность NAT/MAC)
- Для потока из T2 на момент BIND снять `all_entry` и сравнить с tcpdump:
  - sip/dip/sport/dport = тьюпл как на проводе;
  - new_sip/new_dip/new_sport/new_dport = результат после NAT (сверить с
    конфигурацией ip nat / маскарадом и с видимым в SYN после SNAT);
  - MAC (smac/dmac) = MAC интерфейса egress и ARP-соседа/target.
- Любое несовпадение → фиксируем как причину (M3) и правим формат записи.

### T4. ДВУНАПРАВЛЕННЫЙ ПОТОК (iperf в обе стороны, 60 c)
- Проверить, что BIND-записи для обоих направлений появились и поток держится.
- Если рвётся на длинном потоке → смотреть repeat/counters: на каком пакете поток
  умирает; `rx_reasons` — растут ли 0x0e/0x0f после BIND (не должны).

### T5. ДУБЛИКАТЫ (M4)
- На клиенте tcpdump: считать кадры с одними SEQ/длиной на eth0.2 за 10 c iperf.
- Если дублей ~0 — M4 закрыт. Если дублей много прямо на старте потока — M4.

### T6. КОЛЛИЗИИ И СДВИГ (M5)
- Снять `sdk_not_unbind` и `sdk_hash_match`: если not_unbind >> match при активной
  нагрузке — поток встречает уже-BIND слот; есть ли в all_entry записи на idx+1
  (сдвиг должен был быть). Если (BIND)+кэш накапливается → добавить сдвиг как в
  `mtk_check_entry_available`/`mtk_offload_add`.

### T7. СТРЕСС НА ЗАПИСЬ (M2, атомарность)
- iperf3 -P 8 (8 потоков) 120 c; периодически дамп all_entry; одновременно tcpdump.
- Искать «полутаблицы» (state=BIND, но тьюпл/нули/старые MAC/чужой поток) —
  признак races между memset и чтением PPE.
- Если найден state=BIND с чужим/нулевым тьюплом → переход к одной атомарной записи
  (одни wmb + запись state последним, или полный 64B через один memcpy, как
  mtk_foe_write в mtk_offload_add).

### T8. ИТОГОВЫЙ маршрут (по результатам T1–T7):
1. Если T1=2 слота и T2/T3 корректны → вернуть, для проверки, **ограничить bind
   хинт-пакетами** (рабочий вариант 7eb7ab836b) и добавить сдвиг по коллизии;
   затем одним шагом включать безусловный bind.
2. Если T1=1 слот → писать RFC: в bind_hook записывать в слот единственный, но
   заполнять MAC/NAT СТОРОНОЙ ПОТОКА (документировать, какое направление PPE
   для reply использует — вариант общий для SDK в доках на MT7620).

## 20. ТРЕБУЕМЫЕ ДАННЫЕ С КАЖДОГО ПРОГОНА (форма записи)

Для каждого теста сохранять в AI/test_results/STAGE4_*.md блок:
```
=== T<n> дата время boot_uptime ===
-fw: commit <hash> (mtk_offload.c состояние)
-сеть: WAN ip=<ip> mask=<ip> gw=<ip>; LAN=<ip>
-конд: echo-нет; потоки: <описание>
-rx_reasons: 0x0e=<n> 0x0f=<n> bind_hook=<n> bind_hook_entry=<n>
            bind_gate1..8=<n> sdk_fwd/<loc>/hash_hit/unbind/match/mismatch/not_unbind/neigh_fail=<n>
-almentry(num): число BIND=UNBIND=INVALID=
-sлоты: для потока <5-тьюпл> => idx=<...> state=<...> MAC=<...> vlan=<...>
-tcpdump: <первый дельта-фрагмент>
```
Критерий успеха конечной цели: T0 работает; T1/T2/T3 согласуются с одной моделью;
при активном bind (T4–T7) связность и iperf 60 c без потерь, дублей и полутаблиц.

## 21. Сессия 2026-09-10 (P4): WAN-DOWN — гипотеза «виноваты cd2eb4e/259086»

### 21.1 Факты (AI/test_results/STAGE4_W_01.md)

- `eth0.2` существует, link **UP** (logread: «VLAN 'eth0.2' link is up» 15:33:04), но IPv4 **нет**.
- `ubus network.interface.wan`: `up=false, pending=true, proto=dhcp`.
- logread: `udhcpc: sending discover` повторно, **DHCPOFFER не приходит ни одного**.
- Маршрута к `192.168.1.0/24` и default нет → `ping 192.168.1.1` = `Network unreachable` (L3).
- Связность к `192.168.1.1` сейчас не проверяема вообще — независимо от HWNAT. T1–T7 отложены.

### 21.2 Гипотеза пользователя (рабочая, следующая сессия)

WAN не поднялся — **следствие изменений в cd2eb4e + 259086** (bind-every-packet и
изменение drop-политики). Копать в эту сторону.

Подтверждающие соображения:
- Временная связь: на 7eb7ab836b (§12) WAN DHCP 192.168.2.x работал/шёл торрент;
  после двух коммитов WAN-лиase не получается.
- Хук POSTROUTING обрабатывает и LOCAL OUTPUT (sdk_local считает), т.е. путь DHCP.
  (Но пакет DISCOVER может быть сброшен хукoм только при живом хинте — редкий случай;
  OFFER приходит как INPUT, POSTROUTING-hook его не касается.)

Сомнения/что нужно проверить в первую очередь:
1. Прямой механизм блокировки DHCP кодом хука пока не найден (см. выше).
2. Линк eth0.2 поднимается, DISCOVER уходит — значит проблема в приёме OFFER/на
   стороне апстрима; причастность коммитов надо доказать экспериментом.

### 21.3 План следующей сессии (проверка гипотезы)

1. **Эксперимент T0-B**: собрать прошивку, где `mtk_offload_bind_hook()` = `return
   NF_ACCEPT` (тело выключено), залить, проверить, поднимется ли WAN (udhcpc → lease).
   - WAN поднялся → причастность коммитов доказана, дальше локализовать HW-механизм.
   - WAN не поднялся → проблема вне HWNAT (апстрим/кабель/VLAN/udhcpc), закрыть ветку.
2. **Контрольные данные при WANdown**: `tcpdump -i eth0.2 -nn port 67 or port 68 -w wan_dhcp.pcap`
   (выходит ли DISCOVER, приходит ли OFFER), счётчик `sdk_local` за прогон — сырой дамп.
3. При подтверждении причастности: смотреть RX-путь OFFER (sample handling в
   `mtk_offload_check_rx`/`netif_receive_skb`) и поведение PPE для broadcast/unicast UDP:68.

Ссылки: STAGE4_T0_01.md (первичный), STAGE4_W_01.md (WAN-гейт), stage4_wan_gate.md
(задание для локальной модели).
